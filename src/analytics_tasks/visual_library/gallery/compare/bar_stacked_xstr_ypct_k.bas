Sub bar_stacked_xstr_ypct_k()
    ...
    categoryCount = j ' j was last used to count through uniqueCategories in the earlier loop

    ' Copy headers without using the Copy method
    For colIdx = 0 To categoryCount
        ws.Cells(percentageStartRow, outputCol + colIdx).value = ws.Cells(1, outputCol + colIdx).value
    Next colIdx

    ' Calculate percentages for each Y value
    ...
    Set percentDataRange = ws.Range(ws.Cells(percentageStartRow + 1, outputCol), _
                                   ws.Cells(percentageStartRow + totalRow - 2, outputCol + uniqueCategories.Count))
    ...
    For seriesIndex = 1 To chart.SeriesCollection.Count - 1
        ...
        yValueName = ws.Cells(percentageStartRow + seriesIndex, outputCol).value
