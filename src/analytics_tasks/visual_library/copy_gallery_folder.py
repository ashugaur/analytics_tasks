...
def copy_gallery_folder(destination_path):
    ...
    try:
        ...
        try:
            ...
            package_path = pkg_resources.resource_filename('analytics_tasks', 'visual_library/gallery')
        except Exception:
        ...
        if not source_path.is_dir():
            raise FileNotFoundError(f"Gallery path exists but is not a directory: {source_path}")
...
if __name__ == "__main__":
    # Example 1: Copy the entire gallery folder
    try:
        result = copy_gallery_folder("/path/to/your/destination")
        print(f"Gallery copied to: {result}")
    except Exception as e:
        print(f"Failed to copy gallery: {e}")
    
    # Example 2: Copy only the contents of gallery folder
    try:
        result = copy_gallery_contents_only("/path/to/your/destination")
        print(f"Gallery contents copied to: {result}")
    except Exception as e:
        print(f"Failed to copy gallery contents: {e}")