...
## Universal visualization settings
...
def lib_refs_ao_explore(at_dir, report_name=None):
    ...
    automate_office_dir = at_dir / "automate_office"
    visual_library_dir = at_dir / "visual_library"
    input_folder = automate_office_dir / "input"
    input_data_folder = automate_office_dir / "input/data"
    input_img_folder = automate_office_dir / "input/img"
    input_template_folder = automate_office_dir / "input/templates"
    learn_folder = automate_office_dir / "output/learn"
    logs_folder = automate_office_dir / "output/log"
    explore_folder = automate_office_dir / "output/explore"

    result = copy_input_folder(automate_office_dir)
    print(f"Input copied to: {result}")

    _control_file = automate_office_dir / "input/____control.xlsm"
    _control_file_worksheet = "calibration"

    _colors_file = visual_library_dir / "____settings/colors.xlsx"

    _template_path = automate_office_dir / "input/templates/template_v1.potm"
    if report_name:
        _output_pptm = automate_office_dir / rf"/output/{report_name}__{file_dt}.pptm"
    else:
        _output_pptm = automate_office_dir / rf"/output/report__{file_dt}.pptm"
    _xlsm_path = (
        automate_office_dir
        / rf"output/explore/{(Path(_output_pptm).name).rsplit('.')[0]}.xlsm"
    )

    Path(automate_office_dir).mkdir(parents=True, exist_ok=True)
    Path(visual_library_dir).mkdir(parents=True, exist_ok=True)
    Path(input_folder).mkdir(parents=True, exist_ok=True)
    Path(input_data_folder).mkdir(parents=True, exist_ok=True)
    Path(input_img_folder).mkdir(parents=True, exist_ok=True)
    Path(input_template_folder).mkdir(parents=True, exist_ok=True)
    Path(learn_folder).mkdir(parents=True, exist_ok=True)
    Path(logs_folder).mkdir(parents=True, exist_ok=True)
    Path(explore_folder).mkdir(parents=True, exist_ok=True)
...
def process_vba_files():
    ...
    try:
        ...
        try:
            workbook.SaveAs(str(_xlsm_path), 52)  # 52 = xlOpenXMLWorkbookMacroEnabled (XLSM)
...
def replace_values_in_vba_old(vba_code, merged_dict):
    ...
    for key, value in merged_dict.items():
        ...
        if match:
            ...
            # print(f"  Replaced: '{old_text}' with '{replacement}'")
        else:
            print(f"  Warning: Pattern for '{key}' not found in VBA code")
...
def create_or_apply_potm(_template_path, _output_pptm, _control):
    ...
    if os.path.exists(_output_pptm):
        ...
        ppt = ppt_app.Presentations.Open(_output_pptm, WithWindow=False)
    else:
...
def copy_control_file(control_file_path: str, output_file_path: str) -> str:
    ...
    try:
        ...
        # Save as new file
        control_workbook.SaveAs(output_file_path, FileFormat=52)  # 52 = xlsm format
...
def create_excel_charts_batch(
    control_df: pd.DataFrame, control_file_path: str, input_chart_folder=None
):
    """
    ...
    # control_df = control_df[control_df['py_override'] != 1].reset_index(drop=True)

    # Try to access the outer scope's variables
    import sys

    # print("\nDEBUG: Checking available variables in globals:")
    global_vars = list(globals().keys())
    # print(f"Global variables: {[var for var in global_vars if not var.startswith('_')]}")
    ...
    # print("\nDEBUG: Checking available variables in locals:")
    local_vars = list(locals().keys())
    # print(f"Local variables: {[var for var in local_vars if not var.startswith('_')]}")
        ...
        for _, row in chart_rows.iterrows():
            ...
            try:
                ...
                if chart_data_path.endswith(".csv"):
                    ...
                    print(f"REPORT: Auto color column is {_ct_calc}.")
                    current_df = clean_merge(
                        current_df, _colors, df1_join_col=_ct_calc
                    ).reset_index(drop=True)
                    # current_df = pd.merge(current_df, _colors, how='left', left_on=[_ct_calc], right_on=[_ct_default]).reset_index(drop=True)
                else:
                ...
                if current_df is not None:
                    # print("Top 3 rows of processed dataframe:")
                    # print(current_df.head(3))
    ...
    try:
        ...
        for _, row in control_df.iterrows():
            ...
            try:
                ...
                ## Store the returned Shape object from AddPicture
                ...
                #
                # print(f"✅ Inserted image {row['image_link']} to slide {row['to_slide']} and brought to front")
                # else:
                    ...
                    try:
                        ...
                        if os.path.exists(image_path):
                            ...
                            if orig_aspect > ph_aspect:
                                # Image is wider proportionally - set width to match placeholder
                                picture_shape.Width = ph_width
                                # Height will adjust automatically due to locked aspect ratio
                            else:
                                # Image is taller proportionally - set height to match placeholder
                                picture_shape.Height = ph_height
                                # Width will adjust automatically due to locked aspect ratio
                            ...
                            # Optional: Rename the picture shape to match the original placeholder for future reference
                            try:
...
if __name__ == "__main__":
    ...
    # _colors.to_clipboard()



## load_macro_workbook
def load_macro_workbook(explore_folder, _control_file, _control_file_worksheet, visual_library_dir, _xlsm_path):
    _self_run = 1
...
def transform_data(df, x=None, y=None, z=None, value=None):
    ...
    try:
        ...
        if missing_columns:
            raise ValueError(f"Error: The following columns are not found in the provided DataFrame: {missing_columns}. Please check for typos or missing data.")
        ...
        z_cols = ['z'] if z and len(z) == 1 else [f'z{i+1}' for i in range(len(z) if z else 0)]
...
# Example usage
#data = {
    #'date': ['Jan-74', 'Feb-74', 'Mar-74', 'Apr-74'],
    #'mdeaths': [2134, 1863, 1877, 1877],
    #'fdeaths': [901, 689, 827, 677],
    #'region': ['North', 'South', 'East', 'West']
#}
#df = pd.DataFrame(data)
#transform_data(df, x=['date'], y=['mdeaths'])
#transform_data(df, x=['date'], y=['mdeaths'], z=['region'])
#transform_data(df, x=['date'], y=['mdeaths', 'fdeaths'], z=['region'])
