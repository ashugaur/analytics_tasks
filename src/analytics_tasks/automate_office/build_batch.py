...
def lib_refs_ao_batch(at_dir, report_name=None):
    ...
    _template_pathx = _automate_office_dir / "output/template_v1.potm"
    ...
    if len(df_py_override) > 0:
        ...
        for _, row in df_py_override.iterrows():
            ...
            if chart_data_path.endswith(".csv"):
                ...
                current_df = pd.read_csv(chart_data_path)
                current_df = pass_dict_to_transform(current_df, chart_data_dict)
                current_df = transform_data_explore(current_df, _colors_file)
...
def combine_dataframes(elements, elements_master, txt, control, round_columns):
    ...
    # Perform an outer join to preserve all _elements data
    # final_df = elements_master.merge(elements_merged, on=['left', 'top', 'width', 'height'],
    # how='outer', suffixes=('', '_elements'))
    ...
    if missing_elements:
        # s1 = "Missing element_name values in _control:"+ str(missing_elements)

        # s1 = "Unmapped element_name count in _control: "+ str(len(missing_elements))
        # s1x = '-'*len(s1)
        # print('\n\nREPORT\n'+s1x+'\n'+s1+'\n'+s1x+'\n')
...
def get_actual_layout_name(slide):
    try:
        # Check if the layout is a custom layout object
        if hasattr(slide, "CustomLayout"):
            return slide.CustomLayout.Name
        # Check if the layout is a built-in layout integer
