...
def lib_refs_fs(at_dir):
    ...
    logs_folder = at_dir / "file_search/output/log"
    time_machine_path = (
        at_dir / "file_search/input/time_machine"
    )
    reports_folder = at_dir / "file_search/output/report"

    Path(logs_folder).mkdir(parents=True, exist_ok=True)
    Path(time_machine_path).mkdir(parents=True, exist_ok=True)
    Path(reports_folder).mkdir(parents=True, exist_ok=True)

    # Fixed folder to keep search index
    fs_index_dir = at_dir / "file_search/input"

    # Assign the path to the global variable
    global time_machine
    time_machine = time_machine_path

    print("Assigned file_search directories.")

    return fs_index_dir, logs_folder, time_machine_path, reports_folder
...
def scan_time_machine():
    """Checks past index availability."""
    try:
        if (os.path.isfile(time_machine + ".zip") == 0) & (os.path.isdir(time_machine)):
            print("\nNOTE: Directory time_machine exists.")
        elif os.path.isfile(time_machine + ".zip"):
            print("\nREPORT: Loading previous state...")
            # Unzip time_machine.zip
            unzip_file(time_machine + ".zip", time_machine)

            # Delete time_machine.zip
            delete_file(time_machine + ".zip")
    except Exception:
        print(
...
def exceptions(scan, paths_to_exclude):
    ...
    for i in paths_to_exclude:
        print("REPORT: Files to scan before applying exceptions", len(scan))
        scan = scan[
            ~scan["unc"].str.contains(str(i).replace("\\", "/"), na=False, regex=True, case=False)
        ]
        print(f"\tException: {i}")
...
def apply_filters(scan, scan_ext, scan_size):
    ...
    if len(scan) == 0:
        print("WARNING: no new entries available to scan.")
...
def export_index_files(fs_index_dir, scan0, searchx):
    print("REPORT: Exporting final outputs...")
    os.chdir(fs_index_dir)
    ...
    # zip time machine
    zip_folder(time_machine, time_machine / ".zip")

    # delete time machine folder
    delete_folder(time_machine)

    # delete scan.csv
    os.remove(fs_index_dir / "scan.csv")
