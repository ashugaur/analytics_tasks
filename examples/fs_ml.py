# %% SQL codebase classification by information schema using KMeans
...
searchx_sql.glimpse(max_items_per_column=1)
...
info_schema_parsed.glimpse(max_items_per_column=1)
...
sosd = []
K = range(1, 15) # Run clustering for sizes 1 to 15 and capture inertia
for k in K:
...
""" Manually tag the cluster groups to classify files into named categories.
    ...
    Use the classified index to:
        Classify the SQL (in this case) files into categories for easy access.
